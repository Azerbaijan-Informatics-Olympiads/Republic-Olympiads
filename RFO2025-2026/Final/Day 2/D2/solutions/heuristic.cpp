...
struct Basis128 {
  u128 basis[128];
  Basis128() { for (int i = 0; i < 128; ++i) basis[i] = 0; }
  bool insert(u128 x) {
    for (int i = 127; i >= 0; --i) {
      if ((x >> i) & 1) {
        if (!basis[i]) {
...
vector<u128> node_val, edge_weight;

u128 dfs_hash(int u, int p) {
  u128 current_xor = node_val[u];
  for (auto& edge : tree_adj[u]) {
    int v = edge.first, id = edge.second;
    if (v != p) {
      u128 sub_xor = dfs_hash(v, u);
...
u128 get_rand_128(mt19937_64& rng) {
  u128 high = rng();
  u128 low = rng();
  return (high << 64) | low;
}
